      ...
      <section className="grid gap-8 md:grid-cols-[minmax(0,1.05fr)_minmax(0,0.95fr)]">
        <div className="space-y-6">
          <div className="rounded-2xl border border-slate-800 bg-slate-900 p-6 shadow-lg transition-all duration-300">
            <p className="text-sm uppercase tracking-[0.35em] text-teal-300">{t("Dashboard.title")}</p>
            <h1 className="mt-3 max-w-2xl text-4xl font-semibold text-slate-50">{t("Dashboard.heading")}</h1>
            <p className="mt-4 max-w-2xl text-base leading-7 text-slate-300">{t("Dashboard.description")}</p>
          </div>
          ...
          {result ? (
            ...
            <section className="rounded-2xl border border-slate-800 bg-slate-900 p-6 shadow-lg transition-all duration-300">
              <div className="rounded-2xl border border-dashed border-slate-700 bg-slate-950/50 p-10 text-center">
                <p className="text-sm uppercase tracking-[0.35em] text-teal-300">{t("Dashboard.readyWaiting")}</p>
                <h2 className="mt-3 text-2xl font-semibold text-slate-50">{t("Dashboard.resultCardTitle")}</h2>
                <p className="mt-4 text-sm leading-7 text-slate-400">{t("Dashboard.instruction")}</p>
                <div className="mt-8 rounded-2xl border border-slate-800 bg-slate-900/80 p-5 text-left">
                  <p className="text-sm text-slate-300">
                    {isLoading ? t("Dashboard.analysisOngoing") : t("Dashboard.analysisPending")}
                  </p>
                </div>
              </div>
            </section>
